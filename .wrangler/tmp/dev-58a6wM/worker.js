...
// .wrangler/tmp/bundle-hme3bN/strip-cf-connecting-ip-header.js
...
// server/db.ts
var now = /* @__PURE__ */ __name(() => Math.floor(Date.now()), "now");
...
var generateDiscriminator = /* @__PURE__ */ __name(() => Math.floor(1e3 + Math.random() * 9e3).toString(), "generateDiscriminator");
var generateUniqueTag = /* @__PURE__ */ __name(async (db, displayName) => {
  let attempts = 0;
  while (attempts < 10) {
    const discriminator = generateDiscriminator();
    const fullTag = `${displayName}#${discriminator}`;
    const existing = await db.prepare("SELECT uid FROM users WHERE full_tag = ?").bind(fullTag).first();
    if (!existing) {
...
var getAuthUser = /* @__PURE__ */ __name(async (c) => {
  const authHeader = c.req.header("Authorization");
  if (!authHeader?.startsWith("Bearer "))
    return null;
  const token = authHeader.replace("Bearer ", "");
  const session = await c.env.DB.prepare("SELECT uid, expires_at FROM sessions WHERE token = ?").bind(token).first();
  if (!session || session.expires_at < now())
    return null;
  const user = await c.env.DB.prepare("SELECT * FROM users WHERE uid = ?").bind(session.uid).first();
  return user || null;
}, "getAuthUser");
app.post("/api/auth/signup", async (c) => {
  ...
  const existing = await c.env.DB.prepare("SELECT uid FROM users WHERE email = ?").bind(email).first();
  if (existing) {
  ...
  ).bind(uid, displayName, discriminator, fullTag, email, `${salt}:${passwordHash}`, createdAt, createdAt).run();
  const token = crypto.randomUUID();
  const expiresAt = createdAt + 1e3 * 60 * 60 * 24 * 7;
  await c.env.DB.prepare("INSERT INTO sessions (token, uid, created_at, expires_at) VALUES (?, ?, ?, ?)").bind(token, uid, createdAt, expiresAt).run();
  return json({
    user: {
      uid,
      displayName,
...
app.post("/api/auth/login", async (c) => {
  ...
  const createdAt = now();
  const expiresAt = createdAt + 1e3 * 60 * 60 * 24 * 7;
  await c.env.DB.prepare("INSERT INTO sessions (token, uid, created_at, expires_at) VALUES (?, ?, ?, ?)").bind(token, user.uid, createdAt, expiresAt).run();
  return json({
    user: {
      uid: user.uid,
...
app.get("/api/users/search", async (c) => {
  ...
  if (!tag)
    return json({ user: null });
  const user = await c.env.DB.prepare("SELECT * FROM users WHERE full_tag = ?").bind(tag).first();
  if (!user)
    return json({ user: null });
  return json({
    user: {
...
app.post("/api/users/:uid/status", async (c) => {
  ...
  await c.env.DB.prepare("UPDATE users SET status = ?, last_seen = ? WHERE uid = ?").bind(status, now(), uid).run();
  return json({ ok: true });
});
...
app.get("/api/groups", async (c) => {
  ...
  const members = await c.env.DB.prepare(`SELECT group_id, uid, role FROM group_members WHERE group_id IN (${placeholders})`).bind(...ids).all();
...
app.post("/api/groups", async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO groups (id, name, description, created_at, updated_at, is_private, allow_invites, max_members)
       VALUES (?, ?, ?, ?, ?, 1, 1, 50)`
  ).bind(id, name, description, createdAt, createdAt).run();
  await c.env.DB.prepare(
    `INSERT INTO group_members (group_id, uid, role, joined_at, presence, last_active)
       VALUES (?, ?, 'admin', ?, 'online', ?)`
  ).bind(id, user.uid, createdAt, createdAt).run();
  return json({
    group: {
      id,
      name,
...
app.post("/api/groups/:groupId/messages", async (c) => {
  ...
  await c.env.DB.prepare(
    `INSERT INTO messages (id, group_id, sender_id, sender_name, content, type, timestamp)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).bind(id, groupId, user.uid, user.display_name, content, type || "text", timestamp).run();
  return json({
    message: {
      id,
      groupId,
...
app.get("/api/groups/:groupId", async (c) => {
  ...
  if (!group)
    return json({ error: "Not found" }, 404);
  const members = await c.env.DB.prepare("SELECT uid, role FROM group_members WHERE group_id = ?").bind(groupId).all();
  const memberRows = members.results || [];
...
app.post("/api/presence", async (c) => {
  ...
  ).bind(uid, status, groupId || null, now()).run();
  return json({ ok: true });
});
app.post("/api/presence/typing", async (c) => {
  ...
  ).bind(uid, user.status, null, now(), isTyping ? 1 : 0).run();
  return json({ ok: true });
});
...
// .wrangler/tmp/bundle-hme3bN/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-hme3bN/middleware-loader.entry.ts
